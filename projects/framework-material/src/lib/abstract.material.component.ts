...
import {MaterialPromptAdapter} from './prompt.adapter'

/**
 * Angular Material UI 默认提示实现基类。
 *
 * 业务项目可直接继承该基类，以获得 `AbstractPrompt` 的默认实现。
 */
export abstract class AbstractMaterialComponent extends AbstractComponent {
  private readonly promptAdapter = inject(MaterialPromptAdapter)
